...
from ..conus404_helpers import get_accum_types, read_metadata_xtrm
from ..conus404_config import Cfg

from xtrm_to_zarr import load_wrf_files, rechunk_job, resolve_path
pretty.install()
con = Console()
...
def create_empty_zarr(src_zarr: Annotated[Path, Parameter(validator=validators.Path(exists=True))],
    ...
    # TODO: 2024-08-29 PAN - this shouldn't be hardcoded
    ...
    # should be: ['LANDMASK', 'lat', 'lon', 'x', 'y', 'crs']
    ...
    dates = pd.date_range(start=ds.time[0].values, end=end_date, freq='1D')
