...
from ..conus404_helpers import (apply_metadata, build_daily_filelist, delete_dir, get_accum_types,
                                read_metadata_xtrm, rechunker_wrapper)
...
def load_wrf_files(num_days: int,
    ...
    try:
        job_files = build_daily_filelist(num_days, st_date, wrf_dir, file_pat, verify=False)
        ...
        job_files = build_daily_filelist(num_days, st_date, wrf_dir, file_pat, verify=True)
...
def rechunk_job(chunk_index: int,
    ...
    if chunk_index == 0:
        ...
        if len(DROP_VARS) > 0:
            ds_constants = ds_constants.drop_vars(DROP_VARS, errors='ignore')
        ...
        ds_wrf = apply_metadata(ds_wrf, rename_dims, rename_vars, remove_attrs, var_metadata)
    else:
        # The rename_vars variable is only needed for the first rechunk index
        # when the constants file is added.
        ds_wrf = apply_metadata(ds_wrf, rename_dims, {}, remove_attrs, var_metadata)
